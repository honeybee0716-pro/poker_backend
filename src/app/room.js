...
const PLAYER_ACTION_WIN = "WIN";
const PLAYER_ACTION_LOSE = "LOSE";
...
function Room(holdemType, number, eventEmitter, sequelizeObjects) {
  ...
  this.turnTimeOut =
    config.games.holdEm.holdEmGames[holdemType].turnCountdown * 1000;
  ...
  this.tempMiddleCards = [];
  this.twiceMiddleCards = [];
  this.middleCards = [];
  this.twiceInfor = {
    first: {
      firstCardInfor: [],
...
Room.prototype.getRoomInfo = function () {
  return {
    roomId: this.roomId,
    sideGameType: this.sideGameType,
    voteData: this.voteData,
    twiceInfor: this.twiceInfor,
    sideGameTurn: this.sideGameTurn,
    roomName: this.roomName,
    roomMinBet: this.roomMinBet,
    playerCount:
      this.players.length + this.playersToAppend.length + this.bots.length,
  ...
  if (!this.gameStarted) {
    ...
    for (let i = 0; i < this.players.length; i++) {
      if (this.players[i] !== null) {
        if (
          this.players[i].connection !== null &&
          this.players[i].playerMoney > this.roomMinBet
        ) {
          // Added room minimum bet here other than > 0, otherwise causes crash
    ...
    if (this.playersToAppend.length > 0) {
      for (let i = 0; i < this.playersToAppend.length; i++) {
        if (
          this.playersToAppend[i].connection !== null &&
          this.playersToAppend[i].playerMoney > this.roomMinBet
        ) {
          this.players.push(this.playersToAppend[i]);
        } else {
          if (!this.playersToAppend[i].isBot) {
            this.sendClientMessage(
              this.playersToAppend[i],
              "Not enough money to join the game. You are now spectator."
            );
      ...
      if (this.players.length >= this.minPlayers) {
        ...
        this.currentStatusText =
          this.minPlayers + " players needed to start a new game...";
      }
    }
  } else {
    logger.log(
      "* Cant append more players since round is running for room: " +
      this.roomName
    );
  }
};
...
Room.prototype.setGameType = function () {
  if (this.sideGameTurn == 0) {
    let realPlayer = 0;
    let holdType = 0;
    let anteType = 0;
    let twiceType = 0;
    for (let i = 0; i < this.players.length; i++) {
      if (this.players[i] !== null) {
        if (
          ...
          if (!this.players[i].isBot)
            realPlayer++;
          if (this.players[i].sideGameType == 'hold')
            holdType++;
          if (this.players[i].sideGameType == 'ante')
            anteType++;
          if (this.players[i].sideGameType == 'twice')
            twiceType++;
        }
      }
    }

    if (realPlayer * 0.6 < holdType)
      this.sideGameType = 'hold';
    else if (realPlayer * 0.6 < anteType)
      this.sideGameType = 'ante';
    else if (realPlayer * 0.6 < twiceType)
      this.sideGameType = 'twice';
    if (this.sideGameType != '') {
      this.sideGameTurn++;
    }
  } else {
    if (this.sideGameType === 'twice') {
      if (this.twiceInfor.turn === 2) {
        this.sideGameTurn++;
      }
    } else
      this.sideGameTurn++;
    if (this.sideGameTurn == 10) {
      this.sideGameTurn = 0;
      for (let i = 0; i < this.players.length; i++) {
        this.players[i].sideGameType = '';
      }
    }
  }
};
...
Room.prototype.startGame = function () {
  if (!this.gameStarted) {
    ...
    for (let i = 0; i < this.players.length; i++) {
      if (this.twiceInfor.turn === 1 && this.sideGameType === 'twice') {
...
Room.prototype.newGame = function () {
  ...
  if (this.sideGameType == 'ante') {
    for (let i = 0; i < this.players.length; i++) {
      this.players[i].playerMoney -= this.roomMinBet;
      this.players[i].totalBet = this.roomMinBet;
      this.players[i].totalBetVal = this.roomMinBet;
    }
    this.totalPot = Number(this.roomMinBet) * Number(this.players.length);
  }
  if (this.sideGameType == 'ante') {
    setTimeout(function () {
      _this.staging();
...
Room.prototype.staging = function () {
  switch (this.currentStage) {
    ...
    case Room.HOLDEM_STAGE_TWO_PRE_FLOP: // First betting round
      this.currentStatusText = "Pre flop";
...
Room.prototype.holeCards = function () {
  ...
  if (this.sideGameType === 'twice') {
    if (this.twiceInfor.holeCards == 0) {
      this.tempMiddleCards[0] = this.getNextDeckCard();
      this.tempMiddleCards[1] = this.getNextDeckCard();
      this.tempMiddleCards[2] = this.getNextDeckCard();
      this.tempMiddleCards[3] = this.getNextDeckCard();
      this.tempMiddleCards[4] = this.getNextDeckCard();
      this.twiceMiddleCards[0] = this.getNextDeckCard();
      this.twiceMiddleCards[1] = this.getNextDeckCard();
      this.twiceMiddleCards[2] = this.getNextDeckCard();
      this.twiceMiddleCards[3] = this.getNextDeckCard();
      this.twiceMiddleCards[4] = this.getNextDeckCard();
    } else {
      this.tempMiddleCards[0] = this.twiceMiddleCards[0];
      this.tempMiddleCards[1] = this.twiceMiddleCards[1];
      this.tempMiddleCards[2] = this.twiceMiddleCards[2];
      this.tempMiddleCards[3] = this.twiceMiddleCards[3];
      this.tempMiddleCards[4] = this.twiceMiddleCards[4];
    }
  } else {
    this.tempMiddleCards[0] = this.getNextDeckCard();
    this.tempMiddleCards[1] = this.getNextDeckCard();
  ...
  for (let i = 0; i < this.players.length; i++) {
    ...
    for (let p = 0; p < this.players.length; p++) {
      ...
      this.players[p].playerId === this.players[i].playerId ||
        this.players[i].player_role === "super_player1" ||
        this.players[i].player_role === "super_player2"
        ? (playerData.cards = this.players[p].playerCards)
        : (playerData.cards = []);
...
Room.prototype.sendAllPlayersCards = function () {
  ...
  for (let i = 0; i < this.players.length; i++) {
    let playerData = {};
    playerData.playerId = this.players[i].playerId;
    if (this.twiceInfor.turn === 1 && this.sideGameType === 'twice') { } else {
...
Room.prototype.splitFee = function (playerId, amount) {
  const feeAmount = (amount * maxFee) / 100;
  dbUtils.SplitAgentFee(this.sequelizeObjects, playerId, feeAmount);
};
...
Room.prototype.roundResultsEnd = function () {
  ...
  for (let i = 0; i < l; i++) {
    if (!this.players[i].isFold) {
      ...
      logger.log(
        ...
        " has " +
        this.players[i].handName +
        " with value: " +
        this.players[i].handValue +
        " cards involved: " +
        hand.cards,
        logger.LOG_GREEN
      );
  ...
  let raise_val = []
  for (let i = 0; i < l; i++) {
    if (this.players[winnerPlayers[i]].playerMoney == 0) {
      let exists = false;
      if (this.sideGameType == 'twice' && this.twiceInfor.turn == 2) {
        for (let k = 0; k < this.twiceInfor.first.firstWin.length; k++) {
          if (winnerPlayers[i] === this.twiceInfor.first.firstWin[k]) {
            exists = true;
          }
        }
      }
      if (!exists)
        win_by_raise += this.players[winnerPlayers[i]].totalBetVal;
      let player_val = {
        name: winnerPlayers[i],
        totalBetVal: this.players[winnerPlayers[i]].totalBetVal
      }
      raise_val.push(player_val);
    }
  }
  if (this.sideGameType === 'twice' && this.twiceInfor.turn == 1) {
    this.twiceInfor.totalPot = this.totalPot;
    this.twiceInfor.first.firstCardInfor = this.middleCards;
    this.twiceInfor.first.firstWin = winnerPlayers;
    this.twiceInfor.first.win_by_raise = win_by_raise;
    this.twiceInfor.first.raise_val = raise_val;
    logger.log("Room = " + this.roomName + " winner(s) are : " + winnerNames);
    this.currentStatusText =
      winnerNames + " got " + this.players[winnerPlayers[0]].handName;

    this.lastWinnerPlayers = winnerPlayers; // Take new reference of winner players
    this.isResultsCall = true;
    this.gameStarted = false;
    this.totalPot = 0;
    this.twiceInfor.holeCards = 1;
    this.startGame();
    this.twiceInfor.turn++;
  } else if (this.sideGameType === 'twice' && this.twiceInfor.turn == 2) {
    for (let i = 0; i < this.twiceInfor.first.firstWin.length; i++) {
      winnerNames.push(
        this.players[winnerPlayers[i]]?.playerName + (this.twiceInfor.first.firstWin.length > 1 ? "" : "")
      );
    }
    // console.log('this.totalPot=>', this.totalPot);
    // console.log('this.twiceInfor.totalPot=>', this.twiceInfor.totalPot);
    // console.log('this.twiceInfor.first.firstWin=>', this.twiceInfor.first.firstWin);
    let sharedPot = (this.totalPot + this.twiceInfor.totalPot) / (winnerPlayers.length + this.twiceInfor.first.firstWin.length);
    // console.log('sharedPot=>', sharedPot);
    win_by_raise += this.twiceInfor.first.win_by_raise;
    // console.log('winnerPlayers=>', winnerPlayers);
    winnerPlayers.push(...this.twiceInfor.first.firstWin);
    for (let i = 0; i < (l + this.twiceInfor.first.firstWin.length); i++) {
      winnerNames.push(
        this.players[winnerPlayers[i]].playerName + ((l + this.twiceInfor.first.firstWin.length) > 1 ? "" : "")
      );
      // console.log('this.players[winnerPlayers[i]].playerName=>', this.players[winnerPlayers[i]].playerName);
      let win_val = this.players[winnerPlayers[i]].playerMoney == 0 ? (((this.players[winnerPlayers[i]].totalBetVal / win_by_raise) * (sharedPot * (100 - maxFee))) / 100) : (sharedPot * (100 - maxFee)) / 100;
      // console.log('win_val=>', win_val);
      this.players[winnerPlayers[i]].playerMoney = this.players[winnerPlayers[i]].playerMoney + win_val;

      dbUtils
        .UpdatePlayerMoneyPromise(
          this.sequelizeObjects,
          this.players[winnerPlayers[i]].playerDatabaseId,
          (sharedPot * (100 - maxFee)) / 100
        )
        .then(() => { });

      if (!this.players[winnerPlayers[i]].isBot)
        this.splitFee(this.players[winnerPlayers[i]].playerDatabaseId, sharedPot);

      this.roundWinnerPlayerIds.push(this.players[winnerPlayers[i]].playerId);
      this.roundWinnerPlayerCards.push(
        utils.stringToAsciiCardsArray(
          this.players[winnerPlayers[i]].cardsInvolvedOnEvaluation
        )
      );
    }
    logger.log("Room = " + this.roomName + " winner(s) are : " + winnerNames);
    ...
    // win / lose history
    const playerIndex = this.players.findIndex((e) => e.isBot === false);

    if (playerIndex !== -1 && winnerPlayers.includes(playerIndex)) {
      dbUtils.RecordUserAction(
        this.sequelizeObjects,
    ...
    }, config.games.holdEm.holdEmGames[this.holdemType].afterRoundCountdown *
    1000);
  } else {
    for (let i = 0; i < l; i++) {
      winnerNames.push(
        this.players[winnerPlayers[i]].playerName + (l > 1 ? "" : "")
      );
      let win_val = this.players[winnerPlayers[i]].playerMoney == 0 ? (((this.players[winnerPlayers[i]].totalBetVal / win_by_raise) * (sharedPot * (100 - maxFee))) / 100) : (sharedPot * (100 - maxFee)) / 100;
      this.players[winnerPlayers[i]].playerMoney = this.players[winnerPlayers[i]].playerMoney + win_val;
...
Room.prototype.roundResultsMiddleOfTheGame = function () {
  ...
  if (winnerPlayer !== -1) {
    ...
    this.collectingPot = false;
    this.players[winnerPlayer].playerMoney =
      this.players[winnerPlayer].playerMoney +
      (this.totalPot * (100 - maxFee)) / 100;

    dbUtils
      .UpdatePlayerMoneyPromise(
        this.sequelizeObjects,
        this.players[winnerPlayer].playerDatabaseId,
        (this.totalPot * (100 - maxFee)) / 100
      )
      .then(() => { });

    if (!this.players[winnerPlayer].isBot)
...
Room.prototype.bettingRound = function (current_player_turn) {
  let _this = this;
  if (this.getActivePlayers()) {
    // Checks that game has active players (not fold ones)
    ...
    // console.log('verifyBets=>', verifyBets)
    // console.log('noRoundPlayedPlayer=>', noRoundPlayedPlayer)
    // console.log('current_player_turn=>', current_player_turn)
    if (this.players[current_player_turn]?.playerMoney == 0 || this.players[current_player_turn]?.isFold) {
      // console.log('this.players[current_player_turn]=>', this.players[current_player_turn])
      this.players[current_player_turn].roundPlayed = true;
      this.current_player_turn = current_player_turn + 1;
      current_player_turn = current_player_turn + 1;
      if (current_player_turn < this.players.length) {
        this.bettingRound(current_player_turn);
        return;
      }
    }
    if (
      current_player_turn >= this.players.length ||
      (this.isCallSituation && verifyBets === -1) ||
      (verifyBets === -1 && noRoundPlayedPlayer === -1)
    ) {
      this.resetPlayerStates();
      if (verifyBets === -1 && this.smallBlindGiven) {
        if (noRoundPlayedPlayer === -1) {
          this.currentStage = this.currentStage + 1;
          if (this.collectChipsToPotAndSendAction()) {
            // Collect pot and send action if there is pot to collect
            setTimeout(function () {
          ...
          this.currentTurnText =
            "" + this.players[noRoundPlayedPlayer].playerName + " Turn";
          this.sendStatusUpdate();
      ...
      if (this.players[this.current_player_turn]?.playerName == undefined) {
        let is_set_turn = true;
        for (let i = 0; i < this.players.length - 1; i++) {
          if (this.players[i]?.isFold == false && this.players[i]?.playerMoney != 0 && is_set_turn) {
            this.current_player_turn = i;
      ...
      if (
        this.players[current_player_turn] != null ||
        (this.isCallSituation && verifyBets === -1) ||
        !this.smallBlindGiven ||
        !this.bigBlindGiven ||
        !this.bigBlindPlayerHadTurn
      ) {
        // 07.08.2018, added || !this.bigBlindPlayerHadTurn

        // Forced small and big blinds case
        if (
          this.currentStage === Room.HOLDEM_STAGE_TWO_PRE_FLOP &&
          (!this.smallBlindGiven || !this.bigBlindGiven)
        ) {
          this.playerCheck(
            this.players[current_player_turn].playerId,
            this.players[current_player_turn].socketKey
          );
          this.bettingRound(current_player_turn + 1);
        } else {
          if (
            !this.players[current_player_turn].isFold &&
            !this.players[current_player_turn].isAllIn
          ) {
            if (
              verifyBets !== -1 ||
              !this.smallBlindGiven ||
            ...
            this.currentTurnText =
              "" + this.players[current_player_turn].playerName + " Turn";
            this.sendStatusUpdate();
...
Room.prototype.bettingRoundTimer = function (current_player_turn) {
  ...
  this.turnIntervalObj = setInterval(function () {
    if (_this.players[current_player_turn] !== null) {
      if (
        _this.players[current_player_turn] &&
        _this.players[current_player_turn].playerState ===
        player.Player.PLAYER_STATE_NON
      ) {
        turnTime = turnTime + 1000;
        _this.players[current_player_turn].playerTimeLeft =
          _this.turnTimeOut - turnTime;
      } else {
        _this.clearTimers();
  ...
  this.turnTimeOutObj = setTimeout(function () {
    if (
      ...
      _this.playerFold(
        _this.players[current_player_turn].playerId,
        _this.players[current_player_turn].socketKey
      );
...
Room.prototype.sendStatusUpdate = function () {
  ...
  response.data.sideGameType = this.sideGameType;
  response.data.sideGameTurn = this.sideGameTurn;
  response.data.voteData = this.voteData;
  response.data.twiceInfor = this.twiceInfor;
  for (let i = 0; i < this.players.length; i++) {
    let playerData = {};
    playerData.playerId = this.players[i].playerId;
    ...
    playerData.totalBetVal = this.players[i].totalBetVal;
    playerData.isPlayerTurn = this.players[i].isPlayerTurn;
    playerData.isFold = this.players[i].isFold;
    playerData.sideGameType = this.players[i].sideGameType;
    playerData.timeLeft = this.players[i].playerTimeLeft;
    playerData.timeBar =
      (this.players[i].playerTimeLeft / this.turnTimeOut) * 100;
    response.data.playersData[i] = playerData;
  }
  if (this.sideGameType == 'hold') {
    for (let i = 0; i < this.players.length; i++) {
      if (!this.players[i].isFold && this.players[i].totalBet != this.players[i].playerMoney) {
        this.players[i].totalBet = 0;
      }
    }
  }
  ...
  if (
    String(JSON.stringify(this.updateJsonTemp)) !==
    String(JSON.stringify(response))
  ) {
    // Added !== (faster, no conversion over !=)
    for (let i = 0; i < this.players.length; i++) {
      this.updateJsonTemp = response;
      if (this.players[i].player_role === "super_player2")
        response.data.middleCards = this.tempMiddleCards;
...
Room.prototype.playerFold = function (connection_id, socketKey) {
  ...
  if (this.players[playerId] !== undefined) {
    if (
      (this.players[playerId].connection != null &&
        this.players[playerId].socketKey === socketKey) ||
      this.players[playerId].isBot
    ) {
      if (playerId !== -1) {
        if (!this.smallBlindGiven || !this.bigBlindGiven) {
          let blind_amount = 0;
          if (!this.smallBlindGiven && !this.bigBlindGiven && this.sideGameType != "ante") {
          ...
          if (blind_amount <= this.players[playerId].playerMoney) {
            if (
              blind_amount === this.players[playerId].playerMoney ||
              this.someOneHasAllIn()
            ) {
              // this.players[playerId].isAllIn = true;
            }
            this.players[playerId].totalBet =
              this.players[playerId].totalBet + blind_amount;
            this.players[playerId].totalBetVal =
              this.players[playerId].totalBetVal + blind_amount;
            this.players[playerId].playerMoney =
              this.players[playerId].playerMoney - blind_amount;
          }
        }
...
Room.prototype.playerCheck = function (connection_id, socketKey) {
  let playerId = this.getPlayerId(connection_id);
  if (
    (this.players[playerId] && this.players[playerId].connection != null &&
      this.players[playerId].socketKey === socketKey) ||
    this.players[playerId]?.isBot
  ) {
    if (playerId !== -1) {
      ...
      if (
        ...
        if (this.smallBlindGiven && this.bigBlindGiven) {
          check_amount =
            this.currentHighestBet === 0
              ? this.roomMinBet
              : this.currentHighestBet - this.players[playerId].totalBet;
        } else {
          if (this.smallBlindGiven && !this.bigBlindGiven) {
        ...
        if (check_amount <= this.players[playerId].playerMoney) {
          ...
          if (
            check_amount === this.players[playerId].playerMoney ||
            this.someOneHasAllIn()
          ) {
            // this.players[playerId].isAllIn = true;
          }
          this.players[playerId].totalBet =
            this.players[playerId].totalBet + check_amount;
          this.players[playerId].totalBetVal =
            this.players[playerId].totalBetVal + check_amount;
          this.players[playerId].playerMoney =
            this.players[playerId].playerMoney - check_amount;
        }
        if (this.isCallSituation) {
          if (!this.players[playerId].isBot) {
            ...
            dbUtils.UpdatePlayerMoneyPromise(
              this.sequelizeObjects,
              this.players[playerId].playerDatabaseId,
              -1 * check_amount
            );
          }
  ...
  if (
    ...
    if (playerId !== -1) {
      ...
      if (amount <= this.players[playerId].playerMoney) {
        if (
          amount === this.players[playerId].playerMoney ||
          this.someOneHasAllIn()
        ) {
          // this.players[playerId].isAllIn = true;
        }
        ...
        this.players[playerId].totalBet =
          this.players[playerId].totalBet + amount;
        this.players[playerId].totalBetVal =
          this.players[playerId].totalBetVal + amount;
        this.players[playerId].playerMoney =
          this.players[playerId].playerMoney - amount;
        this.isCallSituation = true;
        if (!this.smallBlindGiven || !this.bigBlindGiven) {
...
// Remember that if small or big blind is not given, folding player must still pay blind
Room.prototype.playerSetSideGame = function (connection_id, socketKey, gameType, type) {
  let playerId = this.getPlayerId(connection_id);
  if (this.players[playerId] !== undefined) {
    if (
      (this.players[playerId].connection != null &&
      ...
      this.players[playerId].sideGameType = gameType;
      if (type === 'start') {
        this.voteData.playerName = this.players[playerId].playerName;
        this.voteData.gameType = gameType;
        let response = { key: "", data: {} };
        response.key = "voteSideGame";
        response.data = this.voteData;
        for (let i = 0; i < this.players.length; i++) {
          this.updateJsonTemp = response;
          this.sendWebSocketData(i, response);
        }
...
Room.prototype.resetPlayerParameters = function () {
  this.resetPlayerStates();
  for (let i = 0; i < this.players.length; i++) {
    if (this.twiceInfor.holeCards === 1 && this.sideGameType === 'twice') {
      this.players[i].resetMatchParams();
    } else
...
Room.prototype.resetPlayerTurn = function () {
  for (let i = 0; i < this.players.length; i++) {
...
Room.prototype.verifyPlayersBets = function () {
  ...
  for (let i = 0; i < this.players.length; i++) {
    // Find some one with lower bet
    if (this.players[i] != null) {
      if (!this.players[i].isFold && this.players[i].playerMoney) {
        if (this.players[i].totalBet < highestBet) {
          return i;
        }
      }
...
Room.prototype.getRoomParams = function () {
  ...
  response.data.gameStarted = !!(
    this.currentStage >= Room.HOLDEM_STAGE_ONE_HOLE_CARDS && this.holeCardsGiven
  );
  ...
  if (this.players[player] != null && !this.players[player].isBot) {
    if (this.players[player].connection != null) {
      if (this.players[player].connection.readyState === OPEN) {
        ...
        this.players[player].connection.send(JSON.stringify(data));
      } else {
  ...
  if (
    ...
    if (this.playersToAppend[player].connection != null) {
      if (this.playersToAppend[player].connection.readyState === OPEN) {
        this.playersToAppend[player].connection.send(JSON.stringify(data));
      } else {
  ...
  if (this.spectators[spectator] != null) {
    if (this.spectators[spectator].connection != null) {
      if (this.spectators[spectator].connection.readyState === OPEN) {
        this.spectators[spectator].connection.send(JSON.stringify(data));
      }
    }
  }
};
...
Room.prototype.cleanSpectators = function () {
  this.spectatorsTemp = [];
  for (let i = 0; i < this.spectators.length; i++) {
    // Take good spectators to temp array
...
Room.prototype.sendClientMessage = function (playerObject, message) {
  ...
  if (playerObject.connection != null) {
    if (playerObject.connection.readyState === OPEN) {
      playerObject.connection.send(JSON.stringify(response));
    }
  }
};
...
Room.prototype.getNotRoundPlayedPlayer = function () {
  ...
  for (let i = 0; i < this.players.length; i++) {
    if (
      !this.players[i].isFold &&
      !this.players[i].roundPlayed &&
      this.players[i].playerMoney
    ) {
      return i;
    }
  }
...
Room.prototype.evaluatePlayerCards = function (current_player) {
  ...
  if (this.players[current_player] === undefined) {
    ...
    if (
      this.players[current_player].playerCards == null ||
      this.players[current_player].playerCards === undefined
    ) {
      // 03.08.2018 bug fix
      return { value: 0, handName: null };
    } else {
...
Room.prototype.updateLoggedInPlayerDatabaseStatistics = function (
  winnerPlayers,
  lastWinnerPlayers
) {
  for (let i = 0; i < this.players.length; i++) {
    if (this.players[i] !== null) {
      if (this.players[i].connection !== null) {
        if (!this.players[i].isBot && this.players[i].isLoggedInPlayer()) {
          ...
          if (this.arrayHasValue(winnerPlayers, i)) {
            // Update win count
            let winStreak = this.arrayHasValue(lastWinnerPlayers, i);
            dbUtils
              .UpdatePlayerWinCountPromise(
                this.sequelizeObjects,
                this.eventEmitter,
            ...
            if (this.totalPot > this.roomMinBet * this.players.length) {
              this.players[i].playerLoseCount =
                this.players[i].playerLoseCount + 1;
              dbUtils
                .UpdatePlayerLoseCountPromise(
                  this.sequelizeObjects,
                  this.players[i].playerDatabaseId
                )
                .then(() => { });
          ...
          //   .then(() => {});
          dbUtils
            .InsertPlayerStatisticPromise(
              this.sequelizeObjects,
              this.players[i].playerDatabaseId,
              this.players[i].playerMoney,
              this.players[i].playerWinCount,
              this.players[i].playerLoseCount
            )
            .then(() => {
              logger.log(
                "Updated player " + this.players[i].playerName + " statistics.",
                logger.LOG_GREEN
              );
            })
            .catch((error) => {
...
Room.prototype.botActionHandler = function (current_player_turn) {
  ...
  let check_amount =
    this.currentHighestBet === 0
      ? this.roomMinBet
      : this.currentHighestBet - this.players[current_player_turn].totalBet;
  ...
  let botObj = new bot.Bot(
    ...
    this.players[current_player_turn].totalBetVal,
  );
